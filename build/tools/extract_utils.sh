...
function setup_vendor() {
    ...
    export GLAZE_ROOT="$3"
    if [ ! -d "$GLAZE_ROOT" ]; then
        echo "\$GLAZE_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$GLAZE_ROOT/$OUTDIR" ]; then
        mkdir -p "$GLAZE_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$GLAZE_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$GLAZE_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$GLAZE_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$GLAZE_ROOT"/vendor/glaze/build/tools/smali/baksmali.jar
        export SMALIJAR="$GLAZE_ROOT"/vendor/glaze/build/tools/smali/smali.jar
    fi
    ...
    if [ ! -f "$GLAZE_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$GLAZE_TARGET" >/dev/null; then
...
function extract() {
    ...
    local SRC="$2"
    local OUTPUT_ROOT="$GLAZE_ROOT"/"$OUTDIR"/proprietary
    ...
    for (( i=1; i<COUNT+1; i++ )); do
        ...
        if [ "$SRC" = "adb" ]; then
            ...
            if [ -f "$SRC/$FILE" ]; then
                cp "$SRC/$FILE" "$DEST"
            # if file does not exist try GLAZE target
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$GLAZE_ROOT"/"$OUTDIR"/radio
