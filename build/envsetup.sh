# glaze functions that extend build/envsetup.sh
function __print_glaze_functions_help() {
cat <<EOF
Additional GlazeOS functions:
...
function glaze_device_combos()
{
    local T list_file variant device

    T="$(gettop)"
    list_file="${T}/vendor/glaze/glaze.devices"
    variant="userdebug"
    ...
    do
        add_lunch_combo "glaze_${device}-${variant}"
    done < "${list_file}"
}

function glaze_rename_function()
{
    eval "original_glaze_$(declare -f ${1})"
}

function _glaze_build_hmm() #hidden
{
    printf "%-8s %s" "${1}:" "${2}"
}

function glaze_append_hmm()
{
    HMM_DESCRIPTIVE=("${HMM_DESCRIPTIVE[@]}" "$(_glaze_build_hmm "$1" "$2")")
}

function glaze_add_hmm_entry()
{
    for c in ${!HMM_DESCRIPTIVE[*]}
    do
        if [[ "${1}" == $(echo "${HMM_DESCRIPTIVE[$c]}" | cut -f1 -d":") ]]
        then
            HMM_DESCRIPTIVE[${c}]="$(_glaze_build_hmm "$1" "$2")"
            return
        fi
    done
    glaze_append_hmm "$1" "$2"
}

function glazeremote()
{
    ...
    git remote rm glaze 2> /dev/null
    ...
    git remote add glaze "git@github.com:GlazeOS/$pfx$project"
    echo "Remote 'glaze' created"
}
...
function glaze_push()
{
    ...
    git $path_opt push "ssh://${ssh_name}/GlazeOS/$proj" "HEAD:refs/for/$branch"
}


glaze_rename_function hmm
function hmm() #hidden
{
    local i T
    T="$(gettop)"
    ...
    echo "vendor/glaze extended functions. The complete list is:"
    for i in $(grep -P '^function .*$' "$T/vendor/glaze/build/envsetup.sh" | grep -v "#hidden" | sed 's/function \([a-z_]*\).*/\1/' | sort | uniq); do
        echo "$i"
    done |column
}
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var GLAZE_VERSION)
        ZIPFILE=glaze-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell getprop ro.glaze.device | grep -q "$GLAZE_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $GLAZE_BUILD, run away!"
    fi
}
...
function makerecipe() {
    ...
    then
        pwd
        glazeremote
        git push glaze HEAD:refs/heads/'$1'
    fi
    '
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.glaze.device | grep -q "$GLAZE_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
alias glazekap='dopush glazeka'

function fixup_common_out_dir() {
    common_out_dir=$(get_build_var OUT_DIR)/target/common
    target_device=$(get_build_var TARGET_DEVICE)
    if [ ! -z $GLAZE_FIXUP_COMMON_OUT ]; then
